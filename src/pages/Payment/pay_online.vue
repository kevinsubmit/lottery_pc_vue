<template>
  <div class="payment">
    <h4>
      <i :class="`ico ${(bankName==='快捷支付'||bankName==='扫码支付')?'ico-my-yunpay':bankName==='微信'?'ico-my-wechat': bankName==='支付宝' ? 'ico-my-alipay' : bankName=== '财付通' ? 'ico-my-cai' :bankName=== '网银'?'ico-my-bank':bankName=== '京东'?'ico-my-jd':'' }`"></i>
        ...
        if (res.data.msg == "4001") {
          ...
          switch (this.payType) {
            ...
            case "tenPayOnline":
              ...
              break;
            case "jdPayOnline":
              let max = res.data.jdpay_array[0].max
              let min = res.data.jdpay_array[0].min
