...
<div class="content moneyManage-page accessListBox">
  <nav>
    ...
    <div class="tad">
        <div class="tab el-tabs__item" :class="{act:$index==act}" v-for="(item,$index) in tad" @click="toggleTab(item.tab,$index)">
            <a class="write">{{item.name}}</a>
        </div>
    </div>
    <accessCp :is="currentTab" keep-alive></accessCp>  
  </nav>
</div>
</template>
<script>
...
import scAccess from './scAccess';
import scAccesss from './scAccesss';
...
export default {
    data() {
        return {
            ...
            tad:[
                {name:'彩票存取记录',tab:'accessCp'},
                {name:'体育存取记录',tab:'scAccess'},
                {name:'AG存取记录',tab:'access'},
                // {name:'棋牌存取记录',tab:'scAccesss'}
            ],   
            act:0,
        }
    },
