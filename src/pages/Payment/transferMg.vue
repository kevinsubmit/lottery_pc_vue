		...
		methods: {
			...
			submit(){
				...
				this.$http.post('/newMg/createTransactionByAccountExtRef',JSON.stringify(params)).then(res => {
					...
					} else if(res.data.msg==7003 || res.data.msg==5004){
						loadingInstance.close();
						this.$swal({
							text: "彩票余额不足",
							type: "error",
			...
			isLimitedMoney() {
				if (this.shu == '' || parseInt(this.shu) <= 0 ) {
					...
					return true
				} else if (this.radio == '0' && parseInt(this.shu) > parseInt(this.$store.state.lotteryMoney)) {
					this.$swal({
						text: '彩票余额不足',
						type: "error",
						timer: 1200,
					...
					return true
				} else if (this.radio == '1' && parseInt(this.shu) > parseInt(this.mgBalance)) {
					this.$swal({
						text: 'MG余额不足',
						type: "error",
						timer: 1200,
