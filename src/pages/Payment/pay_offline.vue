<template>
  <div class="payment">
    <h4>
      <i class="ico" :class="getPayIcon()"></i>
    ...
    <div>
      ...
      <div class="tips">
        ...
        一、请在金额转出之后务必填写网页下方的汇款信息表格，以便我们财务人员能及时为您确认添加金额到您的会员账户。 <br> 二、本公司最低存款金额为{{bankmin}}元。
        <br> 此存款信息只是您汇款详情的提交，并非代表存款，您需要自己通过ATM或网银转帐到本公司提供的账户后，填写提交此信息，待工作人员审核充值！
        <br>
      </div>
      ...
      <table class="bank-offline-table userTable">
        ...
        <tr>
          <th>汇款日期</th>
          <td class="dateSelect">
            <el-date-picker v-model="orderDate" type="date" placeholder="选择日期">
            </el-date-picker>
            <el-time-select v-model="orderTime" :picker-options="{
                  start: '08:30',
                  step: '00:15',
                  end: '18:30'
                }" placeholder="选择时间">
        ...
        <tr v-if="isShowOrderNum && bankName != '银行支付'">
          ...
          <td v-else>
            <el-input v-model="orderNo" :minlength="4" :maxlength="4" placeholder="商户单号后四位"></el-input>
          </td>
        </tr>
        ...
        <tr v-if="bankName === '银行支付'">
          <th>备注信息</th>
          <td>
            <el-input v-model="markMsg"></el-input>
          </td>
        </tr>
      ...
      <div class="tips">
        ...
        (1).请按表格填写准确的汇款转账信息,确认提交后相关财务人员会即时为您查询入款情况。 <br> (2).请您在转账金额后面加个尾数,例如:转账金额 1000.66 或 1000.88 等,以便相关财务人员更快确认您的转账金额并充值。 <br> (3).如有任何疑问,您可以联系在线客服,在线客服为您提供365天×24小时不间断的友善和专业客户咨询服务。
        <br>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    getPayIcon(){
      switch(this.payTypeState){
        case '0':
          return 'ico-my-bank'
          break;
        case '1':
          return 'ico-my-alipay'

        case '2':
          return 'ico-my-wechat'
        case '3':
          return 'ico-my-qq'
        case '4':
          return 'ico-my-yunpay'
        case '7':
          return 'ico-my-jd'
        case '6':
          return 'ico-my-union'
    ...
    submit() {
      ...
      if (parseInt(this.amount) > this.bankmax || this.amount < this.bankmin) {
        parseInt(this.amount) > this.bankmax || this.amount < this.bankmin
        // alert(111)
        this.$swal({
        ...
        if (res.data.msg == '4001') {
          ...
          for (let i = 0; i < res.data.linedown_pay_limit.length; i++) {
            if (
              ...
              this.bankName = res.data.linedown_pay_limit[i].payTypeName
              this.bankAddress = res.data.linedown_pay_limit[i].bank_addres
              this.bankUser = res.data.linedown_pay_limit[i].bank_user
              this.bankAccount = res.data.linedown_pay_limit[i].bank_account
              this.codeSrc = res.data.linedown_pay_limit[i].bank_image_url
              this.payTypeState = res.data.linedown_pay_limit[i].pay_type
              if (res.data.linedown_pay_limit[i].bank_image_url == '') {
                this.codeSrc = res.data.linedown_pay_limit[i].bank_image_url2
              }
              this.bankmax = parseInt(res.data.linedown_pay_limit[i].max)
              this.bankmin = parseInt(res.data.linedown_pay_limit[i].min)
              this.bankId = res.data.linedown_pay_limit[i].id
            }
          }
        }
      })
