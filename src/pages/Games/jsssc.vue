<template>
  <div class="content cqssc">
    ...
    <!-- 下注弹窗 -->
    <bet-dialog v-if="showDialog" :betArr="betArr" :game_code=250 :type_code=type_code :round="round" v-on:child-say="listenToMyBoy"></bet-dialog>
  ...
  methods: {
    ...
    fetchData(i) {
      ...
      if (i != 1) {
        ...
        if (
          localStorage.getItem("ssc_list_0") &&
          localStorage.getItem("ssc_list_1") &&
          localStorage.getItem("ssc_list_2") &&
          JSON.parse(localStorage.getItem("ssc_list_0")).msg !== 4001
        ) {
          ...
          this.$http
            .all([this.getOdds(lmp), this.getOdds(gyh), this.getOdds(ydw)])
            .then(
              this.$http.spread((acct, perms, qzhs) => {
