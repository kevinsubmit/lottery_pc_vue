<template>
  <div class="content pk10">
    ...
    <div style='height:80px;'>
      <lottery-area v-if="isLotteryArea" :todayWinLost="todayWinLost" :lotteryName="lotteryName" :game_code=240 :lotteryObj="body" :endtime="endtime" gameType="55" :closetime="closetime" :fentime='fentime' :closeBet="closeBet"></lottery-area>
      <div v-else="isLotteryArea" :class="`bet-top bet-top-placeholder bet-top-placeholder_${getApiName}`">数据加载中...</div>
    </div>

    <!-- 下注弹窗 -->
    <bet-dialog v-if="showDialog" :betArr="betArr" :game_code=240 :type_code=type_code :round="round" v-on:child-say="listenToMyBoy"></bet-dialog>
            ...
            this.$http.spread((acct, perms) => {
              if (acct.status === 200) {
                ...
                localStorage.setItem(
                  "list_jssc_0",
                  JSON.stringify(this.list_jssc_0)
                );
                localStorage.setItem(
                  "list_jssc_1",
                  JSON.stringify(this.list_jssc_1)
                );
              }
            })
          );
          this.$http.all([this.getOdds(ydw), this.getOdds(lds)]).then(
            this.$http.spread((acct, perms) => {
              if (acct.status === 200) {
                ...
                localStorage.setItem(
                  "list_jssc_3",
                  JSON.stringify(this.list_jssc_3)
                );
              }
            })
          );
