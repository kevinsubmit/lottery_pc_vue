<template>
    <div class="content pk10">
     ...
     <div style='height:80px'>
      ...
      <!-- 下注弹窗 -->
      <bet-dialog v-if="showDialog" :betArr="betArr" :game_code=172 :type_code=type_code :round="round"  v-on:child-say="listenToMyBoy"></bet-dialog>
  ...
  methods: {
    ...
    fetchData() {
      ...
      this.classCode='0101'
       ...
       let dxsb = {}
       dxsb.game_code = 172;
       if (localStorage.getItem('jsks_list')) {
          this.list = JSON.parse(localStorage.getItem('jsks_list'));
          ...
          this.$http.post('/getinfo/getAllOdds', JSON.stringify(dxsb)).then(res => {
            ...
            if (res.status == 200) {
              this.list = res.data;
              localStorage.setItem('jsks_list',JSON.stringify(res.data));
