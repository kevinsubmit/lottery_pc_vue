<template>
  <div class="content luckyShip">
    ...
    <div style='height:80px'>
      <lottery-area v-if="isLotteryArea" :todayWinLost="todayWinLost" :game_code=game_code :lotteryName="lotteryName" :lotteryObj="body" :endtime="endtime" gameType="35" :closetime="closetime" :fentime='fentime' :closeBet="closeBet"></lottery-area>
      <div v-else="isLotteryArea" :class="`bet-top bet-top-placeholder bet-top-placeholder_${getApiName}`">数据加载中...</div>
    </div>

    <!-- 下注弹窗 -->
    <bet-dialog v-if="showDialog" :betArr="betArr" :game_code="171" :type_code="type_code" :round="round" v-on:child-say="listenToMyBoy"></bet-dialog>
...
export default {
  ...
  methods: {
    ...
    fetchData(i) {
      ...
      if (i != 1) {
        if (
          localStorage.getItem("ship_list_0") &&
          localStorage.getItem("ship_list_1") &&
          localStorage.getItem("ship_list_3")
        ) {
            ...
            this.$http.spread((acct, perms) => {
              if (acct.status === 200) {
                ...
                localStorage.setItem(
                  "ship_list_0",
                  JSON.stringify(this.list_0)
                );
                localStorage.setItem(
                  "ship_list_1",
                  JSON.stringify(this.list_1)
                );
              }
            })
          );
